...
export default function Networking() {
  const { model, atlas, loading, error } = useApp();
      ...
      {model.networks.length === 0 ? (
        ...
        <div className="card-grid">
          {model.networks.map((net, index) => {
            ...
            const atlasHref = atlasSubjectHref(atlas, net.id);
            return (
              <Panel key={`${net.id}-${index}`} title={routable ? <Link className="entity-detail-link" to={`/networks/${encodeURIComponent(net.name)}`}>{net.name}</Link> : net.name === "" ? UNAVAILABLE_NETWORK : <span className="collision-identity" title={COLLISION_HINT}>{net.name}</span>} icon="network" hint={net.driver === "" ? UNAVAILABLE_NETWORK_DRIVER : net.driver} actions={routable || atlasHref ? <>{routable && <Link className="ghost-link entity-detail-action" aria-label={`Open ${net.name} network detail`} to={`/networks/${encodeURIComponent(net.name)}`}>Open detail</Link>}{atlasHref && <Link className="ghost-link atlas-handoff-link" to={atlasHref}>Open in Atlas</Link>}</> : undefined}>
                ...
                <ul className="rel-list">
                  {net.members.map((member, memberIndex) => {
