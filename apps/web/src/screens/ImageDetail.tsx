...
export default function ImageDetail() {
  ...
  const consumers = record.containers.map((container) => ({ container, service: model.byName.get(container) }));
  ...
  return <div className="screen">
    ...
    <div className="grid-2"><Panel title="Overview" icon="image"><KeyValue label="Image reference" value={record.image} mono /><KeyValue label="Raw aggregate status" value={record.status} mono /><KeyValue label="Consumers" value={record.containers.length} /></Panel><Panel title="Connected containers" icon="service">{consumers.length === 0 ? <p className="muted-line">No consumers in the current snapshot.</p> : <ul className="svc-list">{consumers.map(({ container, service }, index) => <li className="svc-row" key={`${container}-${index}`}><StateDot state={service?.state ?? "unknown"} />{service ? <Link className="svc-name" to={`/services/${encodeURIComponent(service.name)}`}>{container}</Link> : <span>{container}</span>}{service && <Tag tone="muted">{service.state}</Tag>}</li>)}</ul>}</Panel></div>
