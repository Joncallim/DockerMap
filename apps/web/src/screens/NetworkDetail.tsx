...
import { UNAVAILABLE_CONTAINER, UNAVAILABLE_NETWORK_DRIVER, UNAVAILABLE_NETWORK_ID } from "../lib/identity";
...
export default function NetworkDetail({ defaultOpen = false }: { defaultOpen?: boolean }) {
  ...
  const driver = network.driver === "" ? UNAVAILABLE_NETWORK_DRIVER : network.driver;
  const internalsId = "network-internals";
  return <div className="screen">
    <header className="screen-head detail-head"><div className="detail-id"><span className="detail-kind"><Icon name="network" size={18} /></span><div><div className="eyebrow">Docker network</div><h1 className="screen-title">{network.name}</h1></div><Tag tone="muted">{driver}</Tag><Tag tone={network.internal ? "warn" : "accent"}>{network.internal ? "internal" : "not internal"}</Tag></div><Link className="ghost-link" to="/networking">Back to Networking</Link></header>
