...
export default function VolumeDetail({ defaultOpen = false }: { defaultOpen?: boolean }) {
  ...
  const consumers = volume.attachedTo.map((member) => ({ member, service: member ? model.byName.get(member) : undefined }));
  const mounts = consumers.flatMap(({ member, service }) => service ? service.mounts.filter((mount) => mount.kind === "named_volume" && mount.source !== "" && mount.source !== null && (mount.source === volume.name || (volume.id !== "" && mount.source === volume.id))).map((mount) => ({ member, service, mount })) : []);
