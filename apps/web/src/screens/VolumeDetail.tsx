...
export default function VolumeDetail() {
  ...
  const consumers = volume.attachedTo.map((member) => ({ member, service: model.byName.get(member) }));
  const mounts = consumers.flatMap(({ member, service }) => service ? service.mounts.filter((mount) => mount.kind === "named_volume" && (mount.source === volume.name || mount.source === volume.id)).map((mount) => ({ member, service, mount })) : []);
...
function ConsumerList({ consumers }: { consumers: { member: string; service: Service | undefined }[] }) { if (consumers.length === 0) return <p className="muted-line">No connected containers in the current snapshot.</p>; return <ul className="svc-list">{consumers.map(({ member, service }, index) => <li className="svc-row" key={`${member}-${index}`}><StateDot state={service?.state ?? "unknown"} />{service ? <Link className="svc-name" to={`/services/${encodeURIComponent(service.name)}`}>{member}</Link> : <span>{member}</span>}</li>)}</ul>; }
