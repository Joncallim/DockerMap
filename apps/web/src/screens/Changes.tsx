...
function DockerEventObservations({ history }: { history: NonNullable<ReturnType<typeof coherentObservedDockerEvents>> }) {
  const collectionLabel = history.collectionState === "reconnecting"
    ? "reconnecting; observations may be incomplete"
    : history.collectionState;
  return <Panel className="panel-docker-event-observations" title="Docker event observations" icon="history" hint="Observed">
    <p id="docker-event-observation-boundary" className="stream-observation-boundary">
      Bounded daemon-lifetime observations from the read-only Docker event stream. They are separate from current telemetry and snapshot-derived changes; they are not a complete historical record and reconnects can leave gaps.
    </p>
    <p className="stream-collection-state" role="status" aria-live="polite">Collection state: {collectionLabel}</p>
    {history.events.length === 0 ? <EmptyState icon="history" title="No retained stream observations" body="Retention is bounded to the current daemon process lifetime; a reset or reconnect can leave no retained observations." /> : (
      <ol className="stream-observation-list" aria-describedby="docker-event-observation-boundary">
        {history.events.map((event) => <li className="stream-observation-row" key={event.id}>
          <span className="stream-observation-marker" aria-hidden="true"><Icon name="history" size={13} /></span>
          <div className="stream-observation-body"><div className="stream-observation-top"><span className="stream-observation-title">Docker stream observation</span><time className="timeline-time" dateTime={new Date(event.observedAtMs).toISOString()}>{formatRelative(event.observedAtMs)}</time></div>
            <div className="stream-observation-meta"><span>Event kind</span><code>{observedDockerEventKindToken(event.kind)}</code><span>Stream source</span><code>{event.evidenceSource}</code></div>
          </div>
        </li>)}
      </ol>
    )}
