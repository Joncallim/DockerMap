...
export function getDemoResponse<T>(path: string): T {
  ...
  // Docker stream observations are host evidence only. Keep the demo transport
  // explicitly unavailable and empty; the Change Center will not render a
  // stream panel from this response.
  if (pathname === "/api/observed-events") {
    return {
      source: "mock",
