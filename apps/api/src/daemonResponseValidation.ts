...
const V7_EVIDENCE_EDGE = {
  runtime_identity_collision: {
    ...
    // Unlike provider-owned relationship families, V7 is a daemon-created,
    // identity-free aggregate. Its endpoints are literals, never prefixes.
    source: "runtime_integrity_scope",
...
function runtimeEvidenceDiagnostic(payload: unknown): RuntimeEvidenceDiagnostic | null {
  ...
  // V7 is an aggregate claim about the public topology, so it is admissible
  // only when that topology actually contains a duplicate identity. Reserved
  // aggregate endpoints never count: they are generated by this claim rather
  // than evidence for it.
  const hasNonSyntheticIdentityCollision = [...nodesById.entries()].some(([id, matchingNodes]) => (
    id !== "runtime_integrity_scope"
      && id !== "runtime_integrity_risk_identity_collision"
      && matchingNodes.length > 1
  ));
  // A single V7 edge is deliberately the whole collision-cardinality budget.
  // Count it before per-edge validation so a cloned otherwise-valid aggregate
  // fails closed rather than becoming a second browser-visible fact.
  const identityCollisionEdgeCount = edges.filter((candidate) => {
    if (!candidate || typeof candidate !== "object") return false;
    const evidenceRefs = (candidate as { evidenceRefs?: unknown }).evidenceRefs;
    return Array.isArray(evidenceRefs) && evidenceRefs.some((evidence) => (
      evidence && typeof evidence === "object"
        && (evidence as Record<string, unknown>).version === 7
        && (evidence as Record<string, unknown>).provider === "dockermap"
        && (evidence as Record<string, unknown>).kind === "runtime_identity_collision"
    ));
  }).length;
    ...
    for (const evidence of candidate.evidenceRefs) {
      ...
      if (value.subjectRef !== candidate.source
        || (isV7
          ...
          : !candidate.source.startsWith(expected.sourcePrefix))
        || !candidate.target.startsWith(expected.targetPrefix)
        || candidate.source === candidate.target) return "runtime_evidence_source_binding";
      ...
      if (isV7) {
        ...
        if (identityCollisionEdgeCount !== 1 || !hasNonSyntheticIdentityCollision
          || !scope || scope.length !== 1 || !risk || risk.length !== 1
...
function hasCoherentFindings(payload: unknown): boolean {
  ...
  // This conclusion is intentionally one aggregate, never a counter. A
  // coherent summary alone cannot make repeated collision cards acceptable.
  if (findings.filter((candidate) => (
    candidate && typeof candidate === "object"
      && (candidate as Record<string, unknown>).ruleId === RUNTIME_IDENTITY_COLLISION_FINDING_RULE
