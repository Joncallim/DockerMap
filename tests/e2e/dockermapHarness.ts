...
export type Stack = {
  ...
  /** True only when this fixture's generated Docker resources are absent. */
  ownedFixtureResourcesAbsent?: () => boolean;
...
export async function startLiveDockerStack(options: {
  ...
  return {
    ...
    ownedFixtureResourcesAbsent: () => ownedFixtureResourcesAbsent(docker, fixture),
    stop: async () => {
      await stopProcesses(processes);
      cleanupLiveDocker(docker, fixture);
    }
    ...
    if (cleanupError) {
      throw new AggregateError([error, cleanupError], "Live fixture startup and cleanup both failed.");
    }
    throw error;
  }
}
...
function cleanupLiveDocker(docker: string[], fixture: Fixture) {
  ...
  if (!resourcesAbsent) {
    // Keep this category fixed: Docker stderr can contain host identities and
    // must not be copied into test output.
    throw new Error("Owned live Docker fixture cleanup could not be verified.");
  }
}

/**
 * Inspect only names and labels generated by this fixture. No broad Docker
...
function dockerQuiet(docker: string[], args: string[], _cwd: string): string | null {
  const result = spawnSync(docker[0], [...docker.slice(1), ...args], {
    // Fixture cleanup removes fixture.dir before the focused spec performs its
    // independent, closed post-stop check. Use the repository root so that
    // check still observes only the generated name/label filters above.
    cwd: repoRoot,
    encoding: "utf8",
    timeout: 120_000,
  });
