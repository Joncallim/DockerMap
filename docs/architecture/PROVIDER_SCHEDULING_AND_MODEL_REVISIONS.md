...
infrastructure 10 seconds, host-scoped 15 seconds, tmux 15 seconds, cron
15 seconds, systemd 15 seconds, Python processes 10 seconds, native processes
10 seconds, and project npm 60 seconds. At most two slots run at once. There
are no user-configurable timers, persisted telemetry, conditional browser
fetch policy, provider plugin, or policy DSL.
...
2. Network infrastructure (including its fixed opt-in and restricted-PID
   handling).
3. Host-scoped collectors (host node, listeners, PM2).
4. tmux session declarations.
5. Cron declarations.
6. systemd service declarations.
7. Python process projection.
8. Native-process projection.
9. Bounded project-root npm discovery.

Stages 2-9 are the eight independently scheduled `ProviderSlot` values, in the
exact order of `STATIC_PROVIDER_SLOTS`. tmux, cron, and systemd each own an
independent slot and must not inherit host-node, listener, or PM2 freshness.
...
`RuntimeMap.providerStates` is a schema-backed eight-item evidence vector for
the fixed slots: `network_infrastructure`, `host_scoped`, `tmux`, `cron`,
...
host-scoped, 5 tmux, 5 cron, 5 systemd, 7 Python, 7 native, and 2 npm attempts
(initial attempt included). This is a timing/cost comparison only: completed
observations still retain their existing source, redaction, profile, and
...
host-scoped, 5 tmux, 5 cron, 5 systemd, 7 Python, 7 native, and 2 provider
execution opportunities (43 actual slot claims), never more than two concurrently
...
snapshots at all 31 two-second positions, verifies the eight-slot runtime
...
The full-host trace proves 43 scheduler starts and 48 actual child commands
...
host-scoped, tmux, cron, systemd, Python, and native slots become terminal
`disabled` states after their initial profile observation, while the network
and npm slots keep their fixed cadence. This is deterministic command-churn
...
revisions and an eight-item `providerStates` vector; web hook regressions retain
