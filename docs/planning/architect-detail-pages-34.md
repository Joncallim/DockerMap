...
Status: implemented; the binding amendments recorded in Architect pass 2 — collision-safe identity indexes (not first-wins), same-generation model rebuilds, and the single authorized Rust mock-fixture change — supersede the earlier text where they conflict. This document is the binding scope for the implementer; deviation from an arrested lesson is a P1 finding.
...
The shared model is rebuilt and replaced atomically only from a SAME-GENERATION snapshot + runtime-map pair (`useSystemModel.ts:24-30`): mere presence of both resources is not sufficient — while one resource settles ahead of the other, the previous model is kept until the pair realigns, and retained-after-failure data keeps its original generation so a failed resource can never pair with a fresh peer (the hook tests cover both the two-refresh settle-ahead case and the three-refresh failure/realign interleave). During refresh, keep rendering the last model as existing screens do. If an entity disappears or is renamed in a later snapshot, the open detail route changes to the not-found state; do not retain a stale entity object separately.
...
1. **Route keys are correct for normal Docker records, but names are not lossless at the wire-contract boundary.** Live volume collection deliberately copies `volume.name` into both `id` and `name` (`crates/dockermap-daemon/src/main.rs:887-901`), while the mock does not (`crates/dockermap-core/src/lib.rs:1087-1097`); therefore all volume links and lookups must use `name`, and mount matching must use the implemented predicate: a named-volume mount matches only when its source is non-null and non-empty, the source equals the volume name, or the source equals the volume ID only while the volume ID is non-empty (`mount.source !== null && mount.source !== "" && (mount.source === volume.name || (volume.id !== "" && mount.source === volume.id))`). Images are uniquely grouped by exact container image string through a `BTreeMap` (`crates/dockermap-core/src/lib.rs:1110-1134`) and are re-derived for both mock and live caches (`crates/dockermap-daemon/src/main.rs:332-355,409-455`), so `ImageRecord.image` is the only available and normally unique image identity. Network records preserve Docker IDs and names but replace a missing name with the shared literal `unnamed` (`crates/dockermap-daemon/src/main.rs:869-885`); the TypeScript contract itself permits duplicate or empty strings (`packages/contracts/src/index.ts:43-61`). Docker normally enforces engine-scoped uniqueness for network and volume names, but the web model must tolerate malformed/redacted/fixture snapshots rather than pretend route identity is mathematically lossless.

2. **Duplicate-map semantics are collision-safe: collided keys are excluded and reported, never first-wins.** Build `networkByName`, `volumeByName`, and `imageByRef` from the snapshot arrays via `buildIdentityIndex` (`model.ts:321-340`): a non-empty key shared by MORE THAN ONE record is excluded from the index entirely (lookup fails closed — no record, not even the first, is routable under it) and reported in the matching collision set (`networkNameCollisions` / `volumeNameCollisions` / `imageRefCollisions`), so lists render every duplicate row as non-routable text with a collision state and detail routes show a collision screen instead of silently opening the wrong record. A mount or relationship whose identity collided can therefore never emit a detail link, but must stay visible as evidence. Empty identity strings are schema-valid but cannot inhabit a required `:param` and are neither indexed nor collided: render such a row as malformed/unavailable evidence without a detail link, and require every route builder/resolver to reject an empty key. Do not invent a sentinel, fuzzy match, new daemon field, or ID route in this slice. Cross-kind equal strings are harmless because routes and indexes are separate.
...
5. **Runtime label gating is sufficient, intentionally fail-closed, and now backed by a same-generation model guarantee.** Docker runtime node labels are copied directly from snapshot container/network/volume names and carry exact node kinds (`crates/dockermap-core/src/lib.rs:1243-1271,1318-1357`; `packages/contracts/src/index.ts:349-352,399-405`). The daemon computes snapshot and runtime map into one cache refresh (`crates/dockermap-daemon/src/main.rs:409-455`), and although the web fetches `/api/snapshot` and `/api/runtime/map` independently, the model is rebuilt only from a SAME-GENERATION pair (`apps/web/src/hooks/useSystemModel.ts:24-30`): a mismatched pair is never published — the previous model is retained until the generations realign, so a refresh can transiently DELAY publication but can never pair one new resource with one old one. Resolve only when `selected.provider === "docker"`, the exact `selected.type` matches, the label is non-empty, and the corresponding current snapshot map contains it. A renamed/removed node then has no detail link; that fail-closed behavior is acceptable. Never fall back to `selected.service?.name`, `selected.id`, a stripped runtime ID, or a coincidental cross-provider label. Remove both existing service-link sites when installing the one primary resolver (`apps/web/src/screens/Runtime.tsx:70-72,238-242,300-304`).

6. **No half-built detail screen or daemon gap was found.** `ServiceDetail.tsx` is the only current `*Detail.tsx`; routes still stop at the three list pages (`apps/web/src/App.tsx:38-54`). Publication already redacts images, networks, volumes, and mount strings (`crates/dockermap-daemon/src/main.rs:4099-4144`). Pass 1's web-only scope remains binding: routes/screens/model/styles/e2e only; no Express, OpenAPI, contract, collector, or Docker-inspect expansion. The ONE authorized Rust change is the positive encoded-slash mock fixture: `mock_snapshot()` gains the `registry` container and its `ghcr.io/dockermap/example:1.0` image record, plus the two mock-shape test count updates (6 containers, 11 graph nodes) in `crates/dockermap-core/src/lib.rs` — fixture data and count assertions only, no production logic.
...
1. Extend `SystemModel` with `images` plus the three exact collision-safe lookup maps and their collision sets (`buildIdentityIndex`); import `ImageRecord`, build indexes from the snapshot arrays, retain arrays, skip empty keys for routable resolution, and exclude collided non-empty keys so no record is routable under a shared identity.
...
10. Verify diff scope is web/docs/tests only — plus the single authorized Rust mock-fixture change in `crates/dockermap-core/src/lib.rs` (the `registry` container/image fixture and its shape counts) — then run in order: `npm run check`, `npm run test:e2e`, `npm run test:live-docker`. Do not weaken assertions or add daemon data when a relationship is absent; select a truthful fixture record instead.

### Verdict

**Sound with amendments.** Pass 1 chose the correct web-only architecture, source of truth, explicit screens, route encoding, relationship derivations, and test surfaces. The binding amendments are collision-safe duplicate handling (collided keys are excluded from the routing indexes and reported via collision sets instead of first-wins routing), same-generation model rebuilds (a mismatched snapshot/runtime pair is never published; the previous model is retained until the pair realigns), fail-closed empty identities, stronger disclosure a11y, explicit transient Runtime generation mismatch behavior, and fixture-derived live assertions. The only Rust change is the authorized positive encoded-slash mock fixture/count update; no daemon/API/contracts scope expansion is justified.
