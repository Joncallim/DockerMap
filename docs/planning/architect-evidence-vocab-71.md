...
6. **Re-derive the mode pill from the resolved value**, replacing the standalone expression at `AppShell.tsx:146`. Display strings stay in `AppShell` (`"demo" → "Demo"`, `"live" → "Docker"`, `"mock" → "Mock"`, `null → "Mock"` to preserve today's rendering); the *classification* comes only from `resolveEvidenceMode`. Confirm `AppShell.tsx:198` still renders `{mode} Engine` unchanged.
