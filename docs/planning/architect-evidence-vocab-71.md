...
- `apps/web/src/components/AppShell.tsx:224-226` renders the mode as `{modeLabel(evidenceMode)} Engine`, where `modeLabel` (`AppShell.tsx:20-31`) maps `"demo" → "Demo"`, `"live" → "Docker"`, `"mock" → "Mock"`, and `null → "Unknown"` (explicit unknown — never "Mock", per pass-2 amendment 1). The classification comes only from `resolveEvidenceMode` (`AppShell.tsx:161-164`); the pre-slice ternary at the old `AppShell.tsx:146` is deleted (re-verified at remediation HEAD, see "Remediation notes").
- **`health.mode` alone cannot distinguish demo from mock.** `apps/web/src/lib/demoData.ts:174-181` defines `demoHealth` with `mode: "mock"`. In demo mode the browser therefore holds a health object claiming mock. The demo-mode flag must be consulted *first*; the pre-slice pill expression at the old `AppShell.tsx:146` did this by accident of ordering, and the resolver now does it by contract.
- **`health.dockerReachable` is not trustworthy as an evidence authority.** `demoData.ts:177` sets `dockerReachable: true` inside the demo payload, while `apps/api/src/index.ts:470` sets it `false` for the server mock. No code may use `dockerReachable` to decide whether a claim is observed.
- `health` starts as `null` (`apps/web/src/hooks/useDaemonHeartbeat.ts:11`) and only becomes non-null after the first SSE `snapshot` event (`useDaemonHeartbeat.ts:35-39`) or, in demo mode, immediately from `getDemoHealth()` (`useDaemonHeartbeat.ts:15`). There is a real window in live operation where the mode is **not yet known**.
...
- `apps/web/src/lib/settingsStore.ts:127-129`: `isDemoMode()` reads module-global `state`. `settingsStore.ts:39` returns `DEFAULT_SETTINGS` when `typeof window === "undefined"`, and `DEFAULT_SETTINGS.demoMode` is `false` (`settingsStore.ts:27`). **An ambient `isDemoMode()` read therefore fails OPEN to "not demo" under SSR and before hydration** — exactly the wrong direction for a truthfulness primitive.
- `apps/web/src/context.tsx:6-14`: `AppContextValue` = `{ model, loading, error, health, tick, evidenceMode, openCommand }` (carries `evidenceMode` since 92088cc; at pass-1 time it was `{ model, loading, error, health, tick, openCommand }` with no mode). This is the existing per-render distribution channel every screen already uses via `useApp()` (`context.tsx:18-22`).
- `apps/web/src/hooks/useSystemModel.ts:24-30` already pins snapshot and runtime map to the **same generation** before building a model, and retains the previous model otherwise. The mode is a peer input to that pinned tuple.
...
| Home "Updates available" panel | `apps/web/src/screens/Home.tsx:143` (`<Panel title="Updates available" icon="up" hint={`${updates.length}`}>`), list at `Home.tsx:144-159` | Service list asserting pending updates | Same | **No** — `hint` is the count, not a qualifier |
...
| Copilot change answer | `apps/web/src/lib/copilot.ts:167-178` (`changeAnswer`, filters `s.updateAvailable`) | "N services have an update available" as fact | Same | **No** |
...
| Home per-service CPU bar | `apps/web/src/screens/Home.tsx:167` (`resourceFor`) rendered at `Home.tsx:178` (`<Bar value={res.cpuPercent} …/>`) | A filled bar with **no qualifier at all** | Same | **No** |
...
| Causal chain | `apps/web/src/lib/stubs.ts:131-148` (`causalChain`), rendered `apps/web/src/screens/Home.tsx:63` (`<Panel title="What happened" icon="pulse" hint="Causal chain">`) | Narrative inference ("lost its upstream connection") presented as history | Same | **No** — "Causal chain" is a title, not a qualifier |
...
- Gates this slice must fit: `npm run check` → `check:js` (`audit`, `typecheck`, `build`, `test:js`) then `check:rust` (root `package.json:18-24`). `test:e2e` and `test:live-docker` exist (`package.json:32,34`) but are not required by this slice — see DM-02/DM-03 below.

### Could not verify

- **Resolved at remediation HEAD (2026-08-24):** issue #71 WAS read from GitHub, and the sibling numbering #72-#77 was confirmed from the issue list. The scope→number mapping quoted in D10 (#72 updates, #73 resources, #74 history, #75 Copilot, #76 sweep) is confirmed. **#77 is the docs-reconcile slice and is therefore correctly absent from the D10 tagging map** — it is docs-only work with no claim sites to tag.
...
`mock` gets **its own mode value** — it is neither demo nor live, because it reaches the browser through the live code path (`api.ts:29` → real fetch) while carrying no host truth. But it gets **no new kind**: the issue fixes the vocabulary at five, and a sixth kind would fragment every consumer's switch. A host-truth claim in `mock` mode is `demo` kind (it *is* sample data) or `unavailable`. Never `observed`, never `derived`, never `inferred`.
...
- **P2-2 (binding for #72-#76, final gate 2026-08-24):** during the null-authority window — `claimAuthority(null) === "none"`, i.e. before the heartbeat has delivered a snapshot — a site renders the `unavailable` claim's **`detail`** (the concrete reason, e.g. `"Docker daemon unreachable"`), NOT the static `description` `"DockerMap does not collect this yet"`. The description is reserved for **permanent non-collection** (no collector exists for the surface); the detail is for **temporary authority absence** (a collector exists, but no snapshot has arrived). Each sibling slice must pass the reason explicitly to `unavailable(detail)` and must never fall back to the description during the heartbeat window.
...
- **Description vs detail (P2-2, binding for #72-#76):** `"DockerMap does not collect this yet"` is reserved for **permanent non-collection** — no collector exists for the surface. During the null-authority heartbeat window (`claimAuthority(null) === "none"`) a site renders the `unavailable` claim's `detail` (the concrete reason, e.g. `"Docker daemon unreachable"`), never this description.
...
1. `EVIDENCE_KINDS` has exactly 5 entries and equals the union — asserted at type level via the declaration's `as const satisfies readonly EvidenceKind[]` (evidence.ts) plus an exhaustive `Record<EvidenceKind, …>` lookup that fails to compile if a kind is added without strings.
2. Every `label` and every `description` is non-empty after `trim()`, and every `label.length <= 32` (the pass-1 "4." item was merged here when the impl merged the two loops into one `it(...)`). **This is the G-19 gate**: `primitives.tsx:63` suppresses a falsy hint entirely, so an empty label would silently render nothing.
3. `new Set(labels).size === 5` and `new Set(descriptions).size === 5` — distinctness, so two kinds can never read identically to a user.
4. `evidenceLabel("nonsense" as EvidenceKind)` throws — no lenient fallback.

**No silent default / no crossing (the acceptance criterion)**

5. Mode truth table, all 6 combinations of `demoMode ∈ {true, false}` × `healthMode ∈ {"docker", "mock", null}` plus one unrecognized string cast to `RuntimeMode`. Named assertions include: `{demoMode: true, healthMode: "docker"} → "demo"` (demo wins over a docker health — proves demo data can never be classified live) and `{demoMode: false, healthMode: null} → null`.
6. `claimAuthority` over the full mode domain plus `null`: `"live" → "host"`, `"mock" → "sample"`, `"demo" → "sample"`, `null → "none"`. Asserted by iterating `["live","mock","demo",null]` so a future mode value forces a test update.
7. **Positive direction (G-15): the correct behavior RESUMES.** `claimAuthority("live") === "host"` and an `observed(42)` claim in live mode renders `Observed` — the suite must prove live mode still asserts host truth, not merely that demo does not leak.
8. `unavailable("")` and `unavailable("   ")` throw; `unavailable("Live resource collectors are not wired yet")` returns `{ kind: "unavailable", value: null, detail: … }`.
9. Type-level crossing test: a `// @ts-expect-error` line asserting that `claim.value.toFixed(1)` does not compile on an un-narrowed `Claim<number>`. `npm run typecheck` (root `package.json:16`, in `check:js`) makes this a real gate — a `@ts-expect-error` that stops erroring fails the build.
10. The five constructors produce the expected `Claim` shapes (`observed(1)`, `derived(2)`, `inferred(3)`, `demoSample(4)`, `unavailable` covered by 8).

**Micro-render fixtures (delta c + G-19)**

11. For each of the 5 kinds, render `evidenceLabel(kind).label` through each of the three primitives the sibling slices will use — `Panel` `hint` (`primitives.tsx:63`), `Tag` children (`primitives.tsx:28`), and `Metric` `value` (`primitives.tsx:82`) — and assert the label appears in the **visible text** (markup stripped, so a label that drifts into a `title` attribute fails). 15 assertions. This proves no kind is swallowed by the falsy-hint suppression.
12. An `unavailable` claim rendered through `Metric` shows `Not collected` in visible text. (The pass-1 "does not show `0`, `-`, or an empty `metric-value` element" negative asserts were removed at 56a5518 as vacuous — they cannot fail against the statically-typed `value: null`; the G-19 lock is the type-level `value: null` + the live `@ts-expect-error` + these label-presence asserts, and surface-level negative fixtures defer to #72-#76.)
...
`AppShell` is the only place that holds both inputs today — `settings` (`AppShell.tsx:138`) and `health` (`AppShell.tsx:136`) — and it computes the mode via `resolveEvidenceMode` (`AppShell.tsx:161-164`); the pre-slice standalone expression at the old `AppShell.tsx:146` is deleted.

Decision:

1. `AppContextValue` (`context.tsx:6-14`) gains **one required field**: `evidenceMode: EvidenceMode | null`.
2. `AppShell` computes it once per render: `const evidenceMode = resolveEvidenceMode({ demoMode: settings.demoMode, healthMode: health?.mode ?? null });` (`AppShell.tsx:161-164`) and puts it in `ctx` (`AppShell.tsx:177-185`).
3. **The display pill is re-derived from the same value** via `modeLabel` (`AppShell.tsx:224-226`), replacing the independent expression at the old `AppShell.tsx:146`. One mode authority, not two — otherwise the pill and the claim labels can disagree after any future edit to either.
...
- **Live mode must never silently default to `demo`.** In live mode a claim with no evidence source is `unavailable` — a missing collector is not a licence to show a sample. This is D2's `claimAuthority` plus D6's constructor set; there is no code path from `mode === "live"` to a `demo`-kind claim, and test 6 asserts it over the whole mode domain.
...
| #72 updates | `Home.tsx:45` Updates metric, `Home.tsx:143` Updates panel | live | `unavailable` with the `Not collected` label rendered, not a `0` |
...
| #73 resources | `ServiceDetail.tsx:210-212` Resources panel, `Home.tsx:167,178` CPU bar | live | `unavailable`; the Home bar (unqualified today) is the priority site |
...
| #74 history | `stubs.ts:131` `causalChain` → `Home.tsx:63` | demo / mock `demo`; live `unavailable` (or `inferred` **only** if rebuilt from observed states) |
| #75 Copilot | `copilot.ts:167` `changeAnswer` | live | `unavailable` for the change claim |
...
| R3 | **`unavailable` renders as blank, `0`, `-`, or a suppressed hint**, reading as "no load" / "no updates" rather than "not measured". | Type-level: `value: null` on the `unavailable` arm forces narrowing (D6.2, test 9). Render-level: tests 11-12 assert the label text appears in **visible text** through `Panel.hint` (`primitives.tsx:63`, the falsy-suppression site), `Tag`, and `Metric`. (The pass-1 "and that `0` does not" negative renders were removed at 56a5518 as vacuous; negative surface fixtures defer to #72-#76.) |
...
| R5 | **Mode drift** — the pre-slice pill expression (old `AppShell.tsx:146`) and the evidence mode diverge after a later edit, so the UI says "Docker" while claims say sample (or vice versa). | D8.3: the pill at `AppShell.tsx:224-226` is re-derived from `modeLabel(resolveEvidenceMode(...))`; the standalone expression is deleted, not left alongside. |
| R6 | **The demo-health trap resurfaces** — a future contributor reads `health.mode === "mock"` (or `dockerReachable`) directly and classifies demo as server-mock, or demo as reachable. | `resolveEvidenceMode` is the single entry point and checks `demoMode` first with the `demoData.ts:176` citation in a code comment; `dockerReachable` is documented as forbidden (D3) with the `demoData.ts:177` citation. Test 5 pins `{demoMode: true, healthMode: "docker"} → "demo"`. |
...
| Q4 | Demo-mode detection source | `resolveEvidenceMode({ demoMode: settings.demoMode, healthMode: health?.mode ?? null })`, called **once in `AppShell`**, distributed via `AppContext` | `isDemoMode()` (`settingsStore.ts:127-129`) fails open to `false` pre-hydration/SSR (`settingsStore.ts:39`, `:27`). `health.mode` alone cannot see demo (`demoData.ts:176`). `health.dockerReachable` is provably wrong in demo (`demoData.ts:177`). |
| Q5 | How is server-side `mock` classified? | **Its own `EvidenceMode` value**, authority `"sample"`; host-truth claims in mock mode take the `demo` kind or `unavailable` | Mock travels the live code path (`api.ts:29`), so gating on demo alone would let sample data render as observed — the exact defect the epic exists to fix. Classifying it `live` would be a lie; folding it into `demo` would erase the operator-visible distinction at `AppShell.tsx:224-226`. |
...
**G-01 (schema-escape hatches — every schema-valid value must be tolerable to every consumer).** Arrested. The vocabulary's own "schema" is the two unions, and both are closed and exhaustive: `EvidenceKind` is backed by a `Record<EvidenceKind, EvidenceLabel>` (D5), so a new kind without strings is a compile error, and `EVIDENCE_KINDS` is asserted to equal the union (test 1). The escape hatch is a kind cast from untyped data — `evidenceLabel` **throws** there rather than returning a generic label (D5, test 4), so a bad value fails at the boundary instead of rendering a plausible lie. `resolveEvidenceMode` is total over its declared input *and* over unrecognized `RuntimeMode` values (test 5 includes a cast string), returning `null` rather than a mode. `claimAuthority` is total over `EvidenceMode | null` (test 6). `demoMode` itself is validated as a real boolean at the `settingsStore` parse boundary, so a string `"false"` can never reach the resolver (U2).

**G-19 (falsy/empty values need explicit fallbacks at EVERY render site).** Arrested, and this repo contains the live mechanism: `primitives.tsx:63` renders `{hint && <span …>}`, so an empty label silently disappears. Three defences: every `label` and `description` is asserted non-empty after `trim()` (test 2); `unavailable.value` is typed `null` so a numeric render cannot compile without narrowing (D6.2, test 9); and micro-render fixtures render all five kinds through `Panel.hint`, `Tag`, and `Metric` and assert the label is present in **visible text** (tests 11-12; markup stripped, so an attribute-only label fails). The pass-1 "shows `Not collected` and not `0`" wording was updated at remediation: the negative surface asserts were removed at 56a5518 as vacuous against the typed `value: null`, and surface-negative fixtures defer to #72-#76. Each sibling slice repeats the fixture on its own surfaces (D7).

**G-24 (fail CLOSED — missing authority blocks, never an invented lenient default).** Arrested at three points. (a) `resolveEvidenceMode` returns `null` when health has not arrived (`useDaemonHeartbeat.ts:11` starts `health` at `null`) instead of assuming a mode; `claimAuthority(null) === "none"`, so only `unavailable` is permitted during that window — **and (P2-2, binding for #72-#76) in that window a site renders the unavailable claim's `detail` (the reason, e.g. "Docker daemon unreachable"), never the `Not collected` description**, which is reserved for permanent non-collection (no collector exists). (b) Live mode with no evidence source yields `unavailable`, never `demo` — there is no code path from `mode === "live"` to a sample kind, asserted over the whole mode domain (test 6). (c) `evidenceLabel` throws on an unknown kind rather than falling back. The design also removes the existing fail-open read: `isDemoMode()` returns `false` under SSR/pre-hydration (`settingsStore.ts:39`, `:27`) and is forbidden as an evidence input (D3.2).

**G-15 (regression tests must assert correct behavior RESUMES).** Arrested. Test 7 is explicit: the suite asserts `claimAuthority("live") === "host"` and that an `observed` claim renders `Observed`, not merely that demo data fails to leak. Tests 11-12 assert the label text is *present* in visible text, not that a wrong string is absent. Every sibling slice's regression must likewise prove the live surface still renders its real value, not just that the stub is gone — restated in the pass-2 checklist.

**G-23 (no superseded rules left in docs).** Arrested with a scheduled obligation. `stubs.ts:4-14` currently documents the superseded policy ("Every surface that renders this data marks it as estimated") — which is already false at `Home.tsx:167,178`, where the CPU bar carries no notice. #71 does not touch it (Q12), but this doc binds: **when #73 lands, the same commit must update `stubs.ts:4-14` and remove `STUB_NOTICE` (`stubs.ts:16`) and `ResourceSample.estimated` (`stubs.ts:26`); when #74 lands, the same commit must remove `STUB_CHANGES_NOTICE` (`stubs.ts:17`) and `ChangeEvent.estimated` (`stubs.ts:68`).** Pass 2 must grep this document for residual wording if any decision here changes, and each sibling review must grep `stubs.ts` for a header comment that no longer matches the code.
...
**G-08 (fix sweeps introduce regressions — verify prior fixes are CORRECT, not just present).** Arrested prospectively. The consumption map (D10) is the checklist the sibling slices are verified against, and every one of them modifies code that carries earlier hardening fixes: `stubs.ts:93` (collision-safe `routeName`), `stubs.ts:111` (`identityText` normalization), `Home.tsx:148-150` and `Home.tsx:172-174` (the dual `byId`/`byName` link gate), `model.ts:345` (occurrence-safe `dependsOn`). Pass 2 and each sibling review must confirm those behaviors still hold after tagging — the `change-feed-identity`, `collision-identity`, `detail-identity`, and `mount-keys` suites are the guards, and they must keep passing with real assertions, not by fixture weakening.
...
**G-26 (multi-step transactions must pin all derived inputs at start).** Arrested. `evidenceMode` is resolved **once** per render from a pinned input pair and passed down, rather than each claim site re-reading `settings`/`health` at its own moment (D3.2, D8). This is the same discipline `useSystemModel.ts:24-30` already applies to the snapshot/runtime-map pair; when #72 threads the mode into `buildModel`, it must be captured in that same generation-checked memo so a mode flip mid-refresh cannot label a model built from the other mode's data.
...
4. **Add `evidenceMode: EvidenceMode | null` to `AppContextValue`** (`apps/web/src/context.tsx:6-14`) as a **required** field. Typecheck will now fail in 8 places — that is expected and is the point.
5. **Populate it in `AppShell`**: compute `resolveEvidenceMode({ demoMode: settings.demoMode, healthMode: health?.mode ?? null })` (`AppShell.tsx:161-164`) and add it to `ctx` (`AppShell.tsx:177-185`).
6. **Re-derive the mode pill from the resolved value** via `modeLabel` (`AppShell.tsx:20-31`, rendered at `AppShell.tsx:224-226`), replacing the standalone expression at the old `AppShell.tsx:146`. Display strings stay in `AppShell` (`"demo" → "Demo"`, `"live" → "Docker"`, `"mock" → "Mock"`, `null → "Unknown"` (per pass-2 amendment 1 — never "Mock"; genuine mock keeps "Mock")); the *classification* comes only from `resolveEvidenceMode` (`AppShell.tsx:161-164`).
...
1. All file:line citations in "Verified current state" against the branch HEAD — especially the (now deleted) `AppShell.tsx:146`, `demoData.ts:176-177`, `api.ts:30`, `apps/api/src/index.ts:469`, and `primitives.tsx:63`, which carry the load-bearing findings. (Re-verified at remediation HEAD 2026-08-24 — see "Remediation notes".)
...
4. That the old `AppShell.tsx:146` expression was **replaced** (now `modeLabel` at `AppShell.tsx:224-226`), not left duplicated alongside the resolver (R5).
5. That `evidenceLabel` throws rather than falling back, and that no kind's `label` or `description` can be empty (G-01, G-19).
6. That the micro-render fixtures cover all five kinds on all three primitives and assert the label in **visible text**, and that the G-19 lock holds as implemented: type-level `value: null` + live `@ts-expect-error` (test 9) + label-presence asserts (tests 11-12). The pass-1 "`unavailable`-is-not-`0`" assertion was removed at 56a5518 as vacuous (R3).
7. That the suite asserts live behavior **resumes** — not only that demo does not leak (G-15, test 7).
...
12. ~~Whether the GitHub issue #71 body matches `/tmp/dm61-child-1.md`, which this pass could not confirm (see "Could not verify")~~ — **Resolved at remediation HEAD (2026-08-24): issue #71 was read from GitHub; sibling numbering #72-#77 confirmed (see "Could not verify" above).**

## Architect pass 2

### Re-verification summary

> Citations below were verified against the pass-2 HEAD. At remediation HEAD (2026-08-24, union U7/U14) the AppShell lines moved: the ternary is deleted (replaced by `modeLabel`, `AppShell.tsx:20-31`), `resolveEvidenceMode` sits at `AppShell.tsx:161-164`, and the pill is at `AppShell.tsx:224-226`. `modeLabel(null) === "Unknown"` implements pass-2 amendment 1 (finding 1), so loading/error/unreachable now renders "Unknown Engine" — the vacuous-passing consequence for dockermap.spec.ts:29 (below) no longer holds.

- AppShell.tsx:146 — CONFIRMED at pass-2 time: the (since-deleted) ternary `settings.demoMode ? "Demo" : health?.mode === "docker" ? "Docker" : "Mock"`. The interpretive fact recorded then — `health?.mode === "docker"` is false when `health` is undefined/null, so loading/error/unreachable rendered "Mock Engine" — is what pass-2 amendment 1 (finding 1) arrested; `modeLabel` now maps `null → "Unknown"` instead.
- AppShell.tsx:198 — CONFIRMED at pass-2 time (badge was the sole render site of `mode`); the pill is now at `AppShell.tsx:224-226` and remains the sole render site of `modeLabel(evidenceMode)`.
- dockermap.spec.ts:29 (`/Mock Engine/`), :507 (`/Docker Engine/`) — CONFIRMED (paths re-verified at remediation HEAD; the e2e spec lives at `tests/e2e/dockermap.spec.ts`). The pass-2 consequence — ":29 can pass **vacuously during loading** today, because null already renders 'Mock'" — is **resolved by amendment 1**: `modeLabel(null) === "Unknown"`, so `/Mock Engine/` only matches once health genuinely resolves to mock.
...
- context.tsx:5-12 — CONFIRMED at pass-2 time; at remediation HEAD the interface is `context.tsx:6-14` with the required `evidenceMode` field — optional would re-introduce silent absence.
...
### Remediation notes (2026-08-24)

Applied by the #71 union remediation (DeepSeek Flash, one commit per group — code / tests / doc; `npm run check` green at each commit).

### Final gate (Opus, 2026-08-24)

Verdict: **sound-with-amendments**, applied. The final-gate amendments were resolved in this pass, one commit per group (code / tests / doc), `npm run check` green at each commit:

- **P1-1** — `useDaemonHeartbeat` nulls stale demo health at the non-demo branch entry (`setHealth(null)` before the stream opens), so a demo→live flip fails closed to Unknown/offline until the first real snapshot; the error arm leaves the null alone. Locked by the AppShell wiring fixture (demo→live with no health → "Unknown Engine", never "Mock") and the heartbeat transition test (`useDaemonHeartbeat.test.tsx`).
- **P2-3** — the wiring locks in `AppShell.test.tsx`: demo pill + non-pulsing neutral dot (even with `dockerReachable: true` in demo health), mock/live/null pill wiring, and the P1-1 transition.
- **P2-2** — recorded above at D2 / D4 / G-24 and binding for #72-#76: during the null-authority window a site renders the unavailable claim's `detail`, never the `Not collected` description.
- **P3-4** — `evidenceLabel` guards with `Object.hasOwn`; test 4 now throws for `"constructor"` / `"toString"` / `"__proto__"`.
- **P3-5** — `role="status"` on `.conn-msg`; the mode pill keeps its own `role="status"` + `aria-live="polite"`. No e2e selector conflicts (verified).
- **P3-6** — bounded settings validation: `refreshIntervalMs` must be a positive finite number, `auth` must be an object — otherwise each falls back to its default in place; partial-payload loading preserved (the bffc6bb pattern).
- **P3-7** — deferred (next bullet).
- Citations re-verified against final-gate HEAD: every `x.ts:NNN` token in this doc (119 unique citations pre-fix) was checked against the tree; the stale ones (the AppShell pill render-site tokens, the `isDemoMode` tokens, the `ctx` tokens, and the D5 `EVIDENCE_KINDS` snippet) were corrected to `AppShell.tsx:224-226`, `settingsStore.ts:127-129`, `AppShell.tsx:177-185`, and the `as const satisfies readonly EvidenceKind[]` form respectively. Re-checked post-fix: all remaining tokens resolve to real, non-blank lines at final-gate HEAD. Residual: zero actionable.

- **P3-7 (deferred):** the server sends a named `error` frame on stream interruption while the client shows the "Live stream interrupted" copy — the copy mismatch is DEFERRED to the post-merge/#72-#76 sweep; no code change in this slice.

- **U1 decision (P1, type-boundary):** the demo/sample-never-in-live invariant is enforced at the **discipline level** for #71 — mode plumbing (`resolveEvidenceMode`/`claimAuthority`/`evidenceMode` context) + per-surface no-crossing tests in #72-#76; a compile-time type boundary (kind-literal-specialized constructors) was considered and declined for this slice; **revisit at #68/#69**. Opus final gate re-verifies.
- **Citations re-verified (U7/U14) against remediation HEAD.** AppShell lines moved: `modeLabel` at `AppShell.tsx:20-31`, `resolveEvidenceMode` at `AppShell.tsx:161-164`, pill/badge at `AppShell.tsx:224-226`, inputs at `AppShell.tsx:136,138`, `ctx` at `AppShell.tsx:177-185`. Also corrected: `context.tsx:6-14` / `useApp` `context.tsx:18-22`, `useSystemModel.ts:24-30`, `Home.tsx:63` (causal chain panel), `Home.tsx:143`, `Home.tsx:148-150`, `Home.tsx:178`, `copilot.ts:167-178` (`changeAnswer`), `api.ts:29` (`fetchJson`), `apps/api/src/index.ts:470` (server-mock `dockerReachable`), `model.ts:327-330`, `apps/web/package.json:10`, root `package.json:16,18-24,32,34`, and `dockermap.spec.ts:29` → `/Mock Engine/` (e2e spec at `tests/e2e/dockermap.spec.ts`). The deleted ternary is referenced only as "the old `AppShell.tsx:146`".
- **D7/G-19 wording aligned (U6)** with the removed vacuous negative asserts (56a5518): the G-19 lock is the type-level `value: null` + live `@ts-expect-error` (test 9) + visible-text label-presence asserts (tests 11-12); surface-negative fixtures defer to #72-#76.
- **Arithmetic (U11):** the resolver truth table is 6 combinations + 1 unrecognized string, not 8 (D3.2 and D7 item 5).
- **Test numbering (U12):** cross-refs updated to the actual `it(...)` numbers — label-length merged into test 2; `evidenceLabel` throws = test 4, truth table = test 5, `claimAuthority` = test 6, G-15 resume = test 7, `unavailable` detail = test 8, type-level gate = test 9, constructors = test 10 (newly listed).
- **Could-not-verify (U13):** issue #71 WAS read from GitHub; sibling numbering #72-#77 confirmed; #77 (docs-reconcile) is correctly absent from the D10 tagging map.
- **Render oracle (U4):** `evidence-render.test.tsx` asserts on visible text (markup stripped), so a label that drifts into a `title` attribute fails the test.
- **Settings boundary (U2):** `settingsStore.load()` rejects non-object payloads and non-boolean `demoMode` (falls back to defaults, never coerces); `resolveEvidenceMode` documents the boundary.
- **Pill a11y (U3):** `role="status"` + `aria-live="polite"` on the mode pill so the async Unknown → Mock → Docker flip is announced.
- **Demo dot (U5):** the connection dot is neutral (`unknown`, no pulse) in demo mode — no connection can exist there; live/mock keep the `dockerReachable`-driven classes.
- **Heartbeat (U10):** on SSE interruption the hook sets `dockerReachable: false` so the dot drops to offline with the "Live stream interrupted" message (conservative; no other heartbeat semantics touched).
- **Contracts/crates:** `packages/contracts/` and `crates/` remain untouched by the remediation; no public-contract change (D2 non-goal holds).