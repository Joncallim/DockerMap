...
- `apps/web/src/components/AppShell.tsx:217` renders the mode as `{modeLabel(evidenceMode)} Engine`, where `modeLabel` (`AppShell.tsx:20-31`) maps `"demo" → "Demo"`, `"live" → "Docker"`, `"mock" → "Mock"`, and `null → "Unknown"` (explicit unknown — never "Mock", per pass-2 amendment 1). The classification comes only from `resolveEvidenceMode` (`AppShell.tsx:161-164`); the pre-slice ternary at the old `AppShell.tsx:146` is deleted (re-verified at remediation HEAD, see "Remediation notes").
...
2. `AppShell` computes it once per render: `const evidenceMode = resolveEvidenceMode({ demoMode: settings.demoMode, healthMode: health?.mode ?? null });` (`AppShell.tsx:161-164`) and puts it in `ctx` (`AppShell.tsx:173-181`).
3. **The display pill is re-derived from the same value** via `modeLabel` (`AppShell.tsx:217`), replacing the independent expression at the old `AppShell.tsx:146`. One mode authority, not two — otherwise the pill and the claim labels can disagree after any future edit to either.
...
| R5 | **Mode drift** — the pre-slice pill expression (old `AppShell.tsx:146`) and the evidence mode diverge after a later edit, so the UI says "Docker" while claims say sample (or vice versa). | D8.3: the pill at `AppShell.tsx:217` is re-derived from `modeLabel(resolveEvidenceMode(...))`; the standalone expression is deleted, not left alongside. |
...
| Q5 | How is server-side `mock` classified? | **Its own `EvidenceMode` value**, authority `"sample"`; host-truth claims in mock mode take the `demo` kind or `unavailable` | Mock travels the live code path (`api.ts:29`), so gating on demo alone would let sample data render as observed — the exact defect the epic exists to fix. Classifying it `live` would be a lie; folding it into `demo` would erase the operator-visible distinction at `AppShell.tsx:217`. |
...
5. **Populate it in `AppShell`**: compute `resolveEvidenceMode({ demoMode: settings.demoMode, healthMode: health?.mode ?? null })` (`AppShell.tsx:161-164`) and add it to `ctx` (`AppShell.tsx:173-181`).
6. **Re-derive the mode pill from the resolved value** via `modeLabel` (`AppShell.tsx:20-31`, rendered at `AppShell.tsx:217`), replacing the standalone expression at the old `AppShell.tsx:146`. Display strings stay in `AppShell` (`"demo" → "Demo"`, `"live" → "Docker"`, `"mock" → "Mock"`, `null → "Unknown"` (per pass-2 amendment 1 — never "Mock"; genuine mock keeps "Mock")); the *classification* comes only from `resolveEvidenceMode` (`AppShell.tsx:161-164`).
...
4. That the old `AppShell.tsx:146` expression was **replaced** (now `modeLabel` at `AppShell.tsx:217`), not left duplicated alongside the resolver (R5).
...
> Citations below were verified against the pass-2 HEAD. At remediation HEAD (2026-08-24, union U7/U14) the AppShell lines moved: the ternary is deleted (replaced by `modeLabel`, `AppShell.tsx:20-31`), `resolveEvidenceMode` sits at `AppShell.tsx:161-164`, and the pill is at `AppShell.tsx:217`. `modeLabel(null) === "Unknown"` implements pass-2 amendment 1 (finding 1), so loading/error/unreachable now renders "Unknown Engine" — the vacuous-passing consequence for dockermap.spec.ts:29 (below) no longer holds.
...
- AppShell.tsx:198 — CONFIRMED at pass-2 time (badge was the sole render site of `mode`); the pill is now at `AppShell.tsx:217` and remains the sole render site of `modeLabel(evidenceMode)`.
