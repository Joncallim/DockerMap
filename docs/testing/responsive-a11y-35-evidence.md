...
- **Tested application commit:** `ba126660e3702467d3a2461a4e8c5e5c413db0ef` (Sol R2 remediation)
- **Evidence recorded:** 2026-08-22T03:54:22+08:00 (real 200% zoom walkthrough) / evidence updated 2026-08-22T04:55+08:00
...
| --- | --- | --- |
| `npm run test:web` (focused during implementation) | PASS | **90 tests passed across 10 files**: model raw-occurrence preservation (dependencyOccurrences), fail-closed dependency resolution for duplicate `container_*` ids / alias-to-duplicate / unknown `container_*` refs (resolvedId null, no semantic edge or dependent attribution), ServiceMap occurrence-qualified layout (duplicate-id nodes get DISTINCT in-viewport transforms; collision tags inside the 240×240 viewBox), ServiceDetail collision-tagged non-routable rows, change-feed/causal-chain identity fallbacks, ServiceMap/Runtime collision-tag renderers, Diagnostics severity-tone consistency, plus the existing empty/collision/duplicate-key regressions. |
...
| `npm run check` | PASS | `npm audit --omit=dev`: 0 vulnerabilities; typecheck/build/JS tests (90 passed) and Rust fmt/clippy/tests passed. Rust suites: dockermap-core 95 passed, dockermap-daemon 32 passed. |
| `npm run test:e2e` | PASS | **75 passed, 5 opt-in live/production tests skipped**; includes the filtered cross-layer Runtime relation regression (Container layer filter + API→application relation: destination stays selected AND focused, incompatible filter widened); the required immediate re-run passed cleanly. |
