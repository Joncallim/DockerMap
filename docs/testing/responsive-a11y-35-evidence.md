...
- **Tested application commit:** `af4ef5bad37e82c6c2f469dd6698c5eba48d4486` (Sol R3 remediation)
- **Evidence recorded:** 2026-08-22T03:54:22+08:00 (real 200% zoom walkthrough) / evidence updated 2026-08-22T05:15+08:00
...
| --- | --- | --- |
| `npm run test:web` (focused during implementation) | PASS | **93 tests passed across 10 files**: model SOURCE-endpoint uniqueness (a collided/empty source id never enters dependents, dep:* relationships, dependsOn, or impact — its raw occurrence stays visible with resolvedId null, with a unique-source positive control), ServiceMap occurrence-qualified selection (an exact service occurrence highlights ONLY that duplicate-id node; an id-only collided selection is suppressed; the unique id still selects), no-edge-group renderer assertion for the collided source, ServiceMap occurrence-qualified layout (duplicate-id nodes get DISTINCT in-viewport transforms; collision tags inside the 240×240 viewBox), ServiceDetail collision-tagged non-routable rows, plus the existing empty/collision/duplicate-key regressions. |
...
| `npm run check` | PASS | `npm audit --omit=dev`: 0 vulnerabilities; typecheck/build/JS tests (45 API + 93 web + 2 contracts passed) and Rust fmt/clippy/tests passed. Rust suites: dockermap-core 95 passed, dockermap-daemon 32 passed. |
| `npm run test:e2e` | PASS | **76 passed, 5 opt-in live/production tests skipped**; includes the filtered cross-layer Runtime relation regression (Container layer + docker provider filters + API→application relation: ONLY the incompatible layer filter is widened, the compatible provider filter survives, destination stays selected AND focused at the NEXT PAINT) and the new browser-measured viewBox regression (see below); the required immediate re-run passed cleanly. |
